...
/**
 * @param {Object} opts
 * @param {string} opts.svgPath        - SVG path data (M/L commands only)
 * @param {number} opts.boardWidthCm   - Target board width in cm
 * @param {number} opts.boardHeightCm  - Target board height in cm
 * @param {string} opts.bergName       - Mountain name (for comments)
 * @returns {{ sketchCommands: string }}
 */
export function convertSvgPath({ svgPath, boardWidthCm, boardHeightCm, bergName }) {
  ...
  const lines = [`        # Mountain silhouette: ${bergName}`];
  let prevX = null;
  let prevY = null;
