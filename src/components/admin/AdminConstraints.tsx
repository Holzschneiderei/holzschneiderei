...
import type { Constraints, Limits } from '../../types/config';

interface DimensionDiagramProps {
  constr: Constraints;
}

function DimensionDiagram({ constr }: DimensionDiagramProps) {
  return (
...
interface AdminConstraintsProps {
  constr: Constraints;
  setConstrVal: (key: keyof Constraints, val: string) => void;
  limits: Limits;
}

export default function AdminConstraints({ constr, setConstrVal, limits }: AdminConstraintsProps) {
  const pairs: [keyof Constraints, keyof Constraints][] = [["MIN_W", "MAX_W"], ["MIN_H", "MAX_H"], ["MIN_D", "MAX_D"]];
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-1.5 mb-3">
        {[
          { min: "MIN_W" as keyof Constraints, max: "MAX_W" as keyof Constraints, label: "Breite" },
          { min: "MIN_H" as keyof Constraints, max: "MAX_H" as keyof Constraints, label: "Höhe" },
          { min: "MIN_D" as keyof Constraints, max: "MAX_D" as keyof Constraints, label: "Tiefe" },
        ].map(({ min, max, label }) => (
          <div key={min} className="flex items-center gap-1.5">
            <span className="text-[10px] font-semibold text-text w-12 shrink-0">{label}</span>
            <input type="number" min="1" value={constr[min]} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setConstrVal(min, e.target.value)}
            ...
            <input type="number" min="1" value={constr[max]} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setConstrVal(max, e.target.value)}
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 cq-constraint-4">
        {[
          { k: "HOOK_SPACING" as keyof Constraints, l: "Haken-Abstand (cm)" }, { k: "EDGE_MARGIN" as keyof Constraints, l: "Randabstand (cm)" },
          { k: "LETTER_W" as keyof Constraints, l: "Breite/Buchstabe (cm)" }, { k: "LETTER_MARGIN" as keyof Constraints, l: "Schrift-Rand (cm)" },
        ].map(({ k, l }) => (
          <div key={k} className="flex items-center gap-1.5">
            <span className="text-[10px] text-muted flex-1 min-w-0">{l}</span>
            <input type="number" min="1" value={constr[k]} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setConstrVal(k, e.target.value)}
      ...
      <div className="mt-2.5">
        ...
        {[limits.minW, Math.round((limits.minW + limits.maxW) / 2), limits.maxW].filter((w, i, a) => a.indexOf(w) === i).map((w) => {
          ...
          const pct = (n: number) => constr.EDGE_MARGIN / w * 100 + (n * (constr.HOOK_SPACING / w * 100));
          return (
