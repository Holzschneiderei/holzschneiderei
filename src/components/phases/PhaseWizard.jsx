...
export default function PhaseWizard({
  ...
  navDir, animKey, shake, setNavDir, setWizardIndex, setAnimKey,
}) {
  const { form, pricing } = useWizard();
  ...
  return (
    <>
        ...
        <div className="w-full max-w-[720px] flex cq-wizard-body-lg cq-wizard-body-xl">
          ...
          <div className={`w-full max-w-[520px] cq-card-md cq-card-lg cq-card-xl ${shake ? 'animate-shake' : ''}`}>
            <div className="flex justify-between items-center px-0 py-2 mb-2 border-b border-border gap-3">
              ...
              <span className="text-xs text-muted">Schritt {wizardIndex + 1} / {totalSteps}</span>
            </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-[var(--wz-bg,transparent)] border-t border-border z-20 cq-bottom-hide" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
